...
export async function createRecorderBox({
  ...
  async function flushFullSlices() {
    console.log('🔧 [DEBUG] flushFullSlices - accum.length:', accum.length, 'target:', sliceFramesTarget);
    ...
    while (accum.length >= sliceFramesTarget) {
      ...
      console.log('🔧 [DEBUG] Creating chunk:', {
        chunkIndex,
        currentId,
        blobSize: blob.size,
        startMs,
        endMs,
        sliceLength: copy.length
      });

      const result = await storage.putChunk({
